...
def smooth(path, weight_data = 0.5, weight_smooth = 0.1, tolerance = 0.000001):
    ...
    while change >= tolerance:
        ...
        for idx in range(1, len(newpath) - 1):
            for j in range(len(path[0])):

                p0 = path[idx][j]
                aux = newpath[idx][j]
                pti = newpath[idx][j]
                pt_m1 = newpath[idx - 1][j]
                pt_p1 = newpath[idx + 1][j]

                pti += weight_data * (p0 - pti)
                pti += weight_smooth * (pt_p1 + pt_m1 - 2 * pti)
...
for i in range(len(path)):
    print ("[%s] -> [%s]" % (', '.join('%.3f' % x for x in path[i]), ', '.join('%.3f' % x for x in newpath[i])))
