...
# add_constraint2 is fixed for 2 dimensions but could easily be adapted to handle an arbitrary number of dimensions.

def add_constraint(omega, xi, x0, x1, p0, p1, confidence = 1.0):
    if x0 == x1:  # setting initial position
        omega.value[x0][x0] +=  confidence
        xi.value[x0][0] += (p1 - p0) * confidence
    else:
        omega.value[x0][x0] +=  confidence
        omega.value[x0][x1] += -confidence
        omega.value[x1][x0] += -confidence
        omega.value[x1][x1] +=  confidence
        xi.value[x0][0] += (p0 - p1) * confidence
        xi.value[x1][0] += (p1 - p0) * confidence
    return omega, xi

def add_constraint2(omega, xi, x0, x1, p0, p1, confidence = 1.0):
    x0, x1 = x0 * 2, x1 * 2
    omega, xi = add_constraint(omega, xi, x0,     x1,     p0[0], p1[0], confidence)
    omega, xi = add_constraint(omega, xi, x0 + 1, x1 + 1, p0[1], p1[1], confidence)
    return omega, xi

# http://www.udacity-forums.com/cs373/questions/29695/solution-to-6-22-for-n-dimensional-coordinates

def slam3(data, N, num_landmarks, motion_noise, measurement_noise):

    num_coords = 2
    coords = tuple(range(num_coords))
    dim = (N + num_landmarks) * num_coords
    ...
    Xi.zero(dim, 1)
    for c in coords:
        Xi.value[c][0] = world_size / 2.0

    def impl(x1, x2, val, noise):
        Omega.value[x1][x1] += 1.0 / noise
        Omega.value[x2][x2] += 1.0 / noise
        Omega.value[x1][x2] -= 1.0 / noise
        Omega.value[x2][x1] -= 1.0 / noise
        Xi.value[x1][0] -= val / noise
        Xi.value[x2][0] += val / noise

    def measure(x1, x2, measurement):
        impl(x1, x2, measurement, measurement_noise)

    def move(x1, x2, motion):
        impl(x1, x2, motion, motion_noise)

    for di, d in enumerate(data):
        n = di * num_coords
        for measurement in d[0]:
            lm = (N + measurement[0]) * num_coords
            m = measurement[1:]
            for c in coords:
                measure(n + c, lm + c, m[c])
        motion = d[1]
        for c in coords:
            move(n + c, n + num_coords + c, motion[c])

    mu = Omega.inverse() * Xi
    return mu

# http://www.udacity-forums.com/cs373/questions/29378/6-22-yet-another-solution
# http://www.udacity-forums.com/cs373/questions/28191/different-matrix-approach
# http://www.udacity-forums.com/cs373/questions/30051/matrix-operation-version-of-unit-622-solution
...
def slam2(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for d in range(len(data)):
        ...
        for mark in landmarks:
            init_index = d
            final_index = N + mark[0]
            dist = mark[1:]
            print (init_index, "-", final_index)

            add_constraint(init_index, final_index, dist, 1./measurement_noise)
        ...
        final_index = d + 1
        dist = data[d][1]
        add_constraint(init_index, final_index, dist, 1./motion_noise)
...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # bc modelling of X and Y in the same data structure
    dim = 2 * (N + num_landmarks)

    #make the constraint information matrix and vector
    Omega = matrix()
    Omega.zero(dim,dim)
    Omega.value[0][0] = 1.0 #placed in center, has no bearing to final solution
    Omega.value[1][1] = 1.0 # that's why 0, 1 and 1,0 are equal to zero
    ...
    for k in range(len(data)):
        ...
        for i in range(len(landmarks)):
            ...
            for b in range(2):
                ...
                Xi.value [ n + b ] [ 0 ] += -landmarks[i][1+b] / measurement_noise
                Xi.value[m+b][0] += landmarks[i][1+b] / measurement_noise
        ...
        for b in range(2):
            ...
            Xi.value[n+b ][0] += -move[b] / motion_noise
            Xi.value[n+b+2][0] += move[b] / motion_noise
...
N                  = 10       # time steps 20
world_size         = 10.0    # size of world 100
measurement_range  = 2.0     # range at which we can sense landmarks 50
motion_noise       = 2.0      # noise in robot motion #1
measurement_noise  = 2.0      # noise in the measurements #1
distance           = 1.0     # distance by which robot (intends to) move each iteratation 20

data = make_data(N, num_landmarks, world_size, measurement_range, motion_noise, measurement_noise, distance)
result = slam2(data, N, num_landmarks, motion_noise, measurement_noise)
...
#result = slam2(test_data1, 20, 5, 2.0, 2.0)
...
#
#result = slam2(test_data2, 20, 5, 2.0, 2.0)
#print_result(20, 5, result)
#print(result)
#
#
