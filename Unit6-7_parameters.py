...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout and index < len(spath):
        ...
        try:
            projection = (Rx * dx + Ry * dy) / (dx**2 + dy**2)
        except :
            #print("Error: ")
            #print ("Rx = ", Rx, "Ry = ", Ry, "dx = ", dx, "dy = ", dy)
            #index += 1
            return [False]
...
def twiddle(init_params):
    ...
    while sum(dparams) > 0.0000001:
        for i in range(len(params)):
            ...
            if err < best_error:
                best_error = err #float(err) / float(k+1)
                dparams[i] *= 1.1
            else:
